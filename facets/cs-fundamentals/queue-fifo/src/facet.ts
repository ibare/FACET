 ...
 *
 * canonical (`queueFacet`) 과 algorithm / projector / view 를 공유하고 `scenario` 와
 * `layout` 만 다르다. 넣고 빼기 한 번씩이면 "먼저 들어온 것이 먼저 나간다" 는
 * 말이 끝난다.
 *
 * queue 는 coroutine 이라 play 를 눌러야 진행하므로, 컨트롤바를 뗀 대가로
 * `autoStart` 를 선언한다.
 *
 * title / description 은 en·ko 만 채웠다 (발췌 방식 1차 시험).
 */
export const queueFifoFacet: FacetJson = {
  id: 'facet:queueFifo-fifo',
  title: { en: 'First in, first out', ko: '먼저 넣고 먼저 꺼내기' },
  description: {
    en: 'One enters at the back, and the one that entered first leaves at the front',
    ko: '뒤로 하나 들어오고, 가장 먼저 들어와 있던 것이 앞으로 나간다',
  },
  algorithm: 'module:queue',
  projector: 'module:queueProjector',
  initialData: {
    ...
    initialValues: ['A', 'B', 'C'],
    capacity: 10,
    scenario: [{ op: 'enqueue', value: 'D' }, { op: 'dequeue' }],
  },
  shuffleOnReset: false,
  autoStart: true,
  layout: {
    type: 'column',
    gap: 8,
    children: [{ ref: 'stage', padding: '12px 0' }],
  },
  messages: queueFacet.messages,
  blocks: {
    stage: queueFacet.blocks.stage,
  },
};
