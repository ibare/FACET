 ...
 *
 * canonical (`bstFacet`) 과 algorithm / projector / view 를 공유하고 `scenario` 와
 * `layout` 만 다르다. shape 은 시나리오를 비워 초기 트리에서 멈춘다.
 *
 * 셋 다 header · control-bar · 보조 패널을 두지 않는다. 그런데 bst 는 coroutine 이라
 * play 를 눌러야 진행하므로, 컨트롤바를 뗀 대가로 `autoStart` 를 선언해야 한다.
 *
 * title / description 은 en·ko 만 채웠다 (발췌 방식 1차 시험).
 */

export const bstShapeFacet: FacetJson = {
  id: 'facet:bst-shape',
  title: { en: 'Parent and child', ko: '부모와 자식으로 만드는 트리' },
  description: {
    en: 'A tree is nodes tied by parent-child links — nothing is moving yet',
    ko: '트리는 부모와 자식으로 이어진 노드들이다 — 아직 아무것도 움직이지 않는다',
  },
  algorithm: 'module:bst',
  projector: 'module:bstProjector',
  initialData: {
  ...
  messages: bstFacet.messages,
  blocks: {
    stage: bstFacet.blocks.stage,
  },
};

export const bstSearchFacet: FacetJson = {
  id: 'facet:bst-search',
  title: { en: 'Finding by key order', ko: '키 순서로 찾는 트리' },
  description: {
    en: 'Smaller goes left, larger goes right — one comparison halves what is left',
    ko: '작으면 왼쪽 크면 오른쪽 — 비교 한 번이 남은 후보를 반으로 접는다',
  },
  algorithm: 'module:bst',
  projector: 'module:bstProjector',
  initialData: {
...
export const bstInsertFacet: FacetJson = {
  id: 'facet:bst-insert',
  title: { en: 'Where a new key lands', ko: '새 키의 자리가 정해진다' },
  description: {
    en: 'The same comparisons that find a key decide where a new one belongs',
    ko: '찾을 때 하던 비교가 그대로 새 값이 앉을 자리를 정한다',
  },
  algorithm: 'module:bst',
  projector: 'module:bstProjector',
  initialData: {
