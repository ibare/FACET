...
- 이후 추가될 호스트 어댑터 패키지 (`@ffacet/host-markdown`, `@ffacet/host-react` 등)
...
- 호스트 어댑터는 DSL 문법 (`{facet:<id>}`) 파싱을 **자기 파일 안에서** 수행한다. 파서를 `@ffacet/core` 에 집어넣지 않는다. DSL 은 호스트의 관심사.
- 호스트는 `@ffacet/core/runtime` 의 `loadFacet(id)` 또는 `getFacetById(id)` 로 facet JSON 을 조회한다. registry 내부 Map 을 직접 만지지 않는다.
...
- 호스트 어댑터 본체는 `@ffacet/core/runtime` 의 subpath 하나만 의존한다. 구체 algorithm/projector/view/transpiler 패키지에 대한 의존은 **카탈로그 통합 패키지** (`@ffacet/bootstrap`) 또는 **호스트 앱** (playground) 이 진다.
...
- bootstrap 의 facet 등록은 반드시 `registerFacetLoader('facet:<id>', () => import('@ffacet/algorithm-<x>').then(...))` 형식이다. 정적 import 로 algorithm 패키지를 끌어오면 번들러의 dynamic import 경계가 깨져 facet 별 chunk lazy 분리가 무너진다.
...
- rollup 설정은 dynamic import (`import('@ffacet/algorithm-*')`) 가 chunk 경계로 살아남도록 유지한다 (`inlineDynamicImports: false`, manualChunks 로 facet/runtime 명시 분리).
...
- **host-tiptap-bundle 이 `@ffacet/algorithm-*` / `@ffacet/transpiler-*` / `@ffacet/view-*` 를 직접 의존하지 않는다.** 반드시 `@ffacet/bootstrap` 경유.
- bootstrap 이 호스트 어댑터 (`@ffacet/host-tiptap`) 나 외부 배포 번들 (`@ffacet/host-tiptap-bundle`) 을 import 하지 않는다. 의존 방향은 `host-tiptap-bundle → {host-tiptap, bootstrap} → core/runtime + algorithm/view/transpiler` 일방향.

## PREFER

- 마크다운 호스트와 Tiptap 호스트가 DSL 파서 로직을 공유한다면, 공용 파서를 새 패키지 (`@ffacet/host-dsl` 등) 로 분리한다. `@ffacet/core` 에 DSL 을 밀어넣지 않는다.
- 테스트는 `test/extension.test.ts` 처럼 happy-dom + Tiptap 최소 editor 로 DSL → NodeView 렌더 경로를 E2E 로 검증.
- 새 호스트 어댑터를 추가할 때도 카탈로그는 `@ffacet/bootstrap` 을 그대로 재사용한다 (호스트마다 새 카탈로그 패키지를 만들지 않는다).

## Exception

- 테스트 편의를 위해 `test/extension.test.ts` 는 `@ffacet/algorithm-*`, `@ffacet/transpiler-*`, `@ffacet/view-code` 를 devDependencies 로 import 할 수 있다. 테스트 외 경로에서는 금지.
- 호스트 앱 (`apps/playground`) 은 C7 Exception 에 따라 모든 `@ffacet/*` 공개 API 를 자유롭게 import 할 수 있다. 다만 카탈로그 통합은 직접 register* 호출 대신 `@ffacet/bootstrap` 의 `bootstrapFacet()` 을 호출하는 것을 권장한다.
