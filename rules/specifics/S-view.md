...
- **SVG 의 폭은 CSS 가 아니라 속성으로 준다** — `svg.setAttribute('width', '100%')` 또는 `el('svg', { width: '100%' })`. 러너가 만드는 `.facet-block` 슬롯은 `display:flex; flex-direction:column; min-width:0` 이라 부모 폭이 내용으로 정해지는 경우가 있고, 그때 `style.width = '100%'` 는 순환 참조가 되어 브라우저가 SVG 의 기본 intrinsic 폭 **300px** 로 떨어뜨린다. viewBox 가 620 이면 그림이 절반 아래로 눌린다. `maxWidth` 로 상한을 두고 `margin: '0 auto'` 로 가운데 정렬한다.
