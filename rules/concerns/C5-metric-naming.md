...
- 수치 의미가 명확하게 전달되도록 suffix 를 통일: 횟수는 `-count`, 깊이는 `-depth`,
  크기는 `-size`, **합계는 `-sum`**.
  - `-sum` 은 그래프 완제품 배치에서 새로 들어왔다. 같은 배치가 합계에 두 모양을
    만들었기 때문이다 — `weight-sum`(크루스칼)과 `flow-total`(최대 유량). 앞의 셋이
    전부 "무엇을 잰 것인가" 를 명사 접미사로 붙이는 꼴이라 그 결에 맞는 쪽으로
    통일하고 `flow-total` 을 `flow-sum` 으로 고쳤다.
