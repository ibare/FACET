...
export type ConceptBriefing = {
   ...
   * 붙일 만한 자리.
   *
   * **한때 이 필드를 두지 않기로 했었다.** 그 근거는 "언제 쓰는가는 definition
   * 과 observable 에서 도출된다" 였고, 그때는 참이었다 — 개념 하나에 화면이
   * 하나뿐이었으니 "언제 쓰는가" 가 "무엇인가" 와 같은 말이 될 수밖에 없었다.
   *
   * 지금은 아니다. **같은 개념에 화면이 둘 있다.** 한쪽은 개념 하나를 보이고
   * 멈추고, 다른 쪽은 독자가 값을 넣고 몰아 보는 물건이다. 둘은 definition 도
   * exemplarKeywords 도 겹치므로, 무엇을 고를지는 definition 이 원리상 말할 수
   * 없다. 그 자리를 이 필드가 맡는다.
   *
   * ── 쓸 때의 제약 둘
   *
   * 1. **개념마다 다른 문장이어야 한다.** "개념을 설명할 때" / "직접 조작하며
   *    배울 때" 두 문장이 되풀이되면 그것은 분류 필드에 말을 입힌 것이고,
   *    소비자는 그 분류 어휘를 알 이유가 없다. 그 개념에서만 할 수 있는 말을
   *    적는다 — 무엇을 납득시켜야 할 때 이 화면이 필요한가.
   * 2. **글의 구성에 관여하지 않는다.** 몇 개를 쓰라거나 한 절의 얼마를 채우라는
   *    말은 적지 않는다. 우리는 재료를 줄 뿐이고 그것으로 무엇을 어떻게 얼마나
   *    만들지는 쓰는 쪽의 결정이다. (설계 중에 `scope: 'claim' | 'topic'` 을
   *    제안했다가 이 선을 넘은 적이 있다.)
   */
  useWhen: string[];
  /**
   * 붙이면 안 되는 조건.
   *
   * "언제 쓰면 안 되는가" 는 어디에서도 도출되지 않는다 — definition 에
