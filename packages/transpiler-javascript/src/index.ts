...
export const javascriptTranspiler: Transpiler = {
  id: 'javascript',
  language: 'javascript',
  label: { en: 'JavaScript', ko: 'JavaScript' },
  supports: ['imperative'],
  transpile(ir: IR): TranspileResult {
    if (!ir.functions || ir.functions.length === 0) {
      throw new Error(`[transpiler-javascript] IR "${ir.id}" 에 functions 가 비어 있다`);
    }
    ...
    function emitFunc(f: IRFunc): void {
      ...
      lines.push({ code: `function ${f.name}(${params}) {`, phase: null });
...
export function registerJavascriptTranspiler(): void {
  registerTranspiler(javascriptTranspiler.id, javascriptTranspiler);
}
