  ...
  it('알고리즘이 매달리지 않는다', async () => {
    ...
    try {
      ...
      for (let i = 0; i < facets.length; i += BATCH) {
        ...
        //
        // **이 검사는 한 순간만 잰다.** 700 을 820 으로 밀어 보면 여섯이 더
        // 걸린다 (`compareAndSwap` · `digitByDigit` · `heapSortExtract` ·
        // `outOfBounds` · `rotateToBalance` …). 걸음의 길이가 조각마다 달라
        // 어떤 순간에는 매달릴 자리가 열려 있고 어떤 순간에는 닫혀 있는데,
        // 고정된 한 시각은 그중 하나만 본다. 통과가 "매달리지 않는다" 가
        // 아니라 "이 순간에는 매달리지 않는다" 인 셈이다.
        //
        // 여러 시각에서 재도록 넓히는 것은 별개 작업이다 — 지금 걸리는 여섯을
        // 먼저 고쳐야 하고, 그것은 stage 여섯의 애니메이션 대기를 손보는 일이다.
        await delay(700);
